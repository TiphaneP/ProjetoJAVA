...
public class Loja {
    ...
    public Produto produtoMaisVendido() {
        ...
        for (Venda venda : listaDeVendas) {
            for (ItemVenda item : venda.getItens()) {
                Object produto = item.getProduto();
                contagemVendas.put((Produto) produto, contagemVendas.getOrDefault(produto, 0) + item.getQuantidade());
            }
        }
        return contagemVendas.entrySet().stream().max(Map.Entry.comparingByValue()).orElse(null).getKey();
    }

    public Cliente melhorCliente() {
        return listaDeClientes.stream().max(Comparator.comparing(Cliente::getTotalGasto)).orElse(null);
    }
    ...
    public void imprimirResumoVendas() {
        for (Venda venda : listaDeVendas) {
            ...
            for (ItemVenda item : venda.getItens()) {
                System.out.println(" - Produto: " + ((Produto) item.getProduto()).getNome() + ", Quantidade: " + item.getQuantidade() + ", Subtotal: R$ " + item.getSubtotal());
            }
        }
    }
}